...
int main(int argc, char* argv[])
{
    ...
    root = malloc(sizeof(strlen(argv[optind])) + 1);
    strcpy(root, argv[optind]);
    root[strlen(argv[optind])] = '\0';
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char methodtemp[spa1 - line + 1];
            
            strncpy(methodtemp,line, spa1 - line);
            methodtemp[spa1 - line] = '\0';
            char methodCheck[4] = "GET";
            methodCheck[3] = '\0';
            char* method = methodtemp;
            if(strcmp(method, methodCheck) != 0)
            {
                error(405);
                continue;
            }
            ...
            char targetemp[spa2  - spa1 - 1];
          
            strncpy(targetemp,spa1 + 1, (spa2 - spa1 - 1));
            
            targetemp[spa2 - spa1 - 1] = '\0';
            char* target = targetemp;
            ...
            char versiontemp[ctrlf - spa2 - 1];
            
            strncpy(versiontemp,spa2 + 1, (ctrlf - spa2));
            
            versiontemp[ctrlf - spa2 - 1] = '\0';
            char * version = versiontemp;
            if(strcmp(version, "HTTP/1.1") != 0)
            {
                error(505);
                continue;
            }
            if((strchr(method, ' ') != NULL)||(strchr(version, ' ') != NULL)||(strchr(target, ' ') != NULL)) 
            {
                error(400);
                continue;
            }
            ...
            q = strchr(target, '?');
            char querytemp[spa2 - q];
            char apathtemp[q - target];
            char* query = querytemp;

            if(q != NULL)
            {
                strncpy(querytemp, q + 1, spa2 - q - 1);
                querytemp[spa2 - q] = '\0';
                strncpy(apathtemp, targetemp, q - targetemp);
                apathtemp[q - targetemp] = '\0';
                apath = apathtemp;
            ...
            char pathtemp[strlen(apath) + strlen(root) + 1];
            strcpy(pathtemp, root);
            strcat(pathtemp, apath);
            pathtemp[strlen(apath)+ strlen(root)] = '\0';
            char* path = pathtemp;
     
            // TODO: extract path's extension
            char* bdot = strchr(path, '.');
            if(bdot == NULL)
            {
                error(400);
                continue;
            }
            char extensiontemp[target - bdot + 1];
            strcpy(extensiontemp, bdot + 1);
            char* extension = extensiontemp;
            
            
            // TODO: ensure path exists
            int est = access(path, F_OK);
            if(est == -1)
            {
                error(404);
                continue;
